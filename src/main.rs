...
impl Food {
    ...
    pub fn change_color(&mut self, rng: &mut Rand32) {
        ...
        match rng.rand_range(1..6) {
            1 => food_color = Color::GREEN,
            2 => food_color = Color::BLUE,
            3 => food_color = Color::GREEN,
            4 => food_color = Color::RED,
            5 => food_color = Color::MAGENTA,
            6 => food_color = Color::CYAN,
            _ => food_color = Color::WHITE,
        }
...
impl event::EventHandler<ggez::GameError> for GameState {
                ...
                if let Some(ate) = self.snake.ate {
                    ...
                    match ate {
                        ...
                        Ate::Food => {
                            ...
                            for item in self.food.iter_mut().enumerate() {
                                if item.1.pos == self.snake.head.pos {
                                    item.1.pos = new_food_pos;
                                    item.1.change_color(&mut self.rng)
                                }
                            }
    ...
    fn draw(&mut self, ctx: &mut Context) -> GameResult {
        ...
        self.snake.draw(ctx)?;
        for item in self.food.iter().enumerate() {
            item.1.draw(ctx)?;
        }
